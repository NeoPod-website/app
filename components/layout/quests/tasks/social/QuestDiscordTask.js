...
const QuestDiscordTask = ({ task, questId, discordUser }) => {
  ...
  // Handle verifying membership
  const handleVerifyMembership = async () => {
    try {
      setLoading(true);

      const guildId = task?.guildId; // Use custom guild ID if provided

      // Always use the verify endpoint - backend handles role assignment based on guild
      const endpoint = `${process.env.NEXT_PUBLIC_API_URL}/socials/discord/verify`;
      ...
      dispatch(
        updateTaskAnswer({
          ...
          answer: {
            ...
            roleAssigned: data.roleAssigned || null,
          },
        }),
      );

      const successMessage = data.roleAssigned
        ? `Membership verified and ${data.roleAssigned} role assigned!`
        : `Discord membership verified successfully!`;
        ...
        <div className="flex items-center gap-2 rounded-xl border border-gray-300 px-3 py-2">
          ...
          <p className="text-sm text-gray-100">
            {isTaskCompleted
              ...
              : `Join ${serverData?.name || task?.serverName || "our Discord server"} and verify membership to complete this quest.`}
          </p>
        </div>
