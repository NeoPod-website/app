...
const QuestDetails = async ({ quest }) => {
  ...
  if (quest.conditions && quest.conditions.length > 0) {
    const requiredQuestIds = quest.conditions
  ...
  // Determine what to show based on availability FIRST
  const shouldShowSubmissionForm = availabilityResult.available;

  // Only show submission status if quest is NOT available AND has relevant submission
  const shouldShowSubmissionStatus =
    !availabilityResult.available && // Quest must NOT be available first
    mostRecentSubmission &&
    // Has pending submission that's blocking availability
    ((["pending", "in_progress"].includes(mostRecentSubmission.review_status) &&
      availabilityResult.reason === "Submission pending review") ||
      // Quest blocked by recurrence (already submitted this period)
      availabilityResult.reason.includes("Already submitted this") ||
      // Quest blocked by cooldown
      availabilityResult.reason === "Quest in cooldown period" ||
      // Quest completed (one-time only)
      availabilityResult.reason === "Quest already completed (one-time)");
