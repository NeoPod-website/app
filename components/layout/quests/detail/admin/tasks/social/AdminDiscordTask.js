...
const DiscordServerSettings = ({ taskId }) => {
  ...
  const [localGuildId, setLocalGuildId] = useState(currentTask.guildId || "");

  const debouncedInviteLink = useDebounce(localInviteLink, 500);
  const debouncedServerName = useDebounce(localServerName, 500);
  const debouncedGuildId = useDebounce(localGuildId, 500);

  useEffect(() => {
    if (currentTask) {
      setLocalInviteLink(currentTask.inviteLink || "");
      setLocalServerName(currentTask.serverName || "");
      setLocalGuildId(currentTask.guildId || "");
    }
  }, [currentTask]);
  ...
  useEffect(() => {
    if (!taskId) return;
    if (debouncedGuildId !== currentTask.guildId) {
      safeUpdateTask({ guildId: debouncedGuildId });
    }
  }, [debouncedGuildId, currentTask.guildId, taskId]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <p className="mt-1 text-xs italic text-gray-300">
          Leave empty for NEO server (with role assignment). Add Guild ID for
          other servers (membership verification only).
        </p>
      </div>

      <div className="bg-blue-900/20 rounded-lg border border-blue-400 p-3">
        <p className="text-sm text-blue-300">
          <strong>Note:</strong> Ambassadors will always need to verify Discord
          membership. NEO server assigns roles automatically, other servers only
          verify membership.
        </p>
      </div>
    </div>
