...
const AdminCategoryContainer = ({ category }) => {
  return (
    <div
      key={category.id}
      className="group relative flex-1 rounded-2.5xl bg-black/50"
    >
      <CategoryItem
        id={category.id}
      ...
      <div className="absolute left-0 top-0 z-30 flex h-full w-full items-center justify-center rounded-2.5xl border border-gray-300 bg-black/60 opacity-0 backdrop-blur transition-opacity group-hover:opacity-100">
        <Link
